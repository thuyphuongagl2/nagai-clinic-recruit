...
function createMessage(
  ...
  row.innerHTML = `
    <tr>
      <td class="severity ${severity === 'warning' ? 'warning' : 'error'}">${severity === 'warning' ? '⚠' : '✖'
    }</td>
...
class HTMLHint extends HTMLElement {
  constructor(data?: HTMLHintResult[]) {
    super()
    if (!data) return
    ...
    if (errored) {
      ...
      data.forEach((err) => {
        ...
        err.warnings.forEach((hint) => {
          ...
          const spaces = repeatStr(String(line).length + pointCol + 3)
          row.className = 'row'
          row.innerHTML = `
            <div class="evidence">L${line} |${escapeHtml(evidence)}</div>
            <div class="message">${spaces}^ ${hint.message
            } <span class="rule">(${hint.rule.id})</span></div>
          `
...
class Stylelint extends HTMLElement {
  constructor(data?: NewStylelintResult[]) {
    super()
    if (!data) return
    const errored = data.some((item) => item.warnings.length > 0)
    if (errored) {
...
class ESLint extends HTMLElement {
  constructor(data?: NewESLintResult[]) {
    super()
    if (!data) return
    const errored = data.some((item) => item.messages.length > 0)
    if (errored) {
...
function createErrorOverlay(data: LintStorage) {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (value.length) {
      switch (key) {
        case 'htmlhint':
          body.appendChild(new HTMLHint(value as HTMLHintResult[]))
          break
        case 'stylelint':
          body.appendChild(new Stylelint(value as NewStylelintResult[]))
          break
        case 'eslint':
          body.appendChild(new ESLint(value as NewESLintResult[]))
          break
...
function clearErrorOverlay() {
  ; (
    document.querySelectorAll(overlayId) as NodeListOf<LintErrorOverlay>
  ).forEach((n) => n.close())
}
...
if (import.meta.hot) {
  ...
  import.meta.hot.on('lint:results', (data: LintStorage) => {
    const errored = Object.values(data).some((results) =>
      results.some(
        (result) =>
          (result as HTMLHintResult | NewStylelintResult).warnings?.length ||
          (result as NewESLintResult).messages?.length,
      ),
    )
    if (errored && !hasError) {
